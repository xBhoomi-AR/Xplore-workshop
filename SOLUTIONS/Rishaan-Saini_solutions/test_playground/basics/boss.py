...
while choice =='y' or choice =='Y' : # make 'Y' valid too
    try:
        ...
        numbers =list(map(float, input("Enter the input numbers separated by spaces: ").split())) # split by space and convert to list
        ...
        if len(numbers) != len(operators) + 1: # this seems odd... u might say it's ... off by one
            print("Do it right bruv ? ") # replace wiht better message :)
            continue
    ...
    except Exception as e: # catch all exceptions, also print the exception for better debugging
...
while choice =='y' or choice =='Y' : # make 'Y' valid too
    try:
        ...
        print(f"Evaluated result is: {eval(expr)}") 
    except Exception as e: # catch all exceptions, also print the exception for better debugging
        print(f"Exception: {e}") # print exception
    finally:
        choice = input("Do you want to continue? [y/n] : ") # always ask before ending