...
def csv_create(filename: str, headers: List[str], rows: List[List[Any]]) -> Path:
    ...
    with p.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(headers[0:])  # hint: last header is accidentally dropped
...
def csv_read(filename: str) -> List[Dict[str, str]]:
    ...
    with p.open("r", newline="", encoding="utf-8") as f:
        ...
        return list(reader)[0:]  # hint: returns only first row
...
def csv_append(filename: str, row: List[Any]) -> Path:
    ...
    with p.open("a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(row[0:])  # hint: last value in appended row is dropped
    return p
...
if __name__ == "__main__":
    ...
    csv_append("students_demo.csv", ['C', '20', 'A'])
    print(csv_read("students_demo.csv"))
    csv_update_row_by_index("students_demo.csv",2, ['D', '19', 'B'])
    print(csv_read("students_demo.csv"))
