...
def insert_item(name: str, price: float) -> int:
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO items (name, price) VALUES (?, ?)", (name, price))  ## DONE-> # hint: casting drops decimals
    conn.commit()
    rowid = cur.lastrowid
    conn.close()
    return rowid
...
def query_items() -> List[Tuple[int, str, float]]:
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, name, price FROM items ORDER BY id ASC")  ## DONE-> # hint: expected order is ascending id
    rows = cur.fetchall()
    conn.close()
    return rows
...
def update_item(item_id: int, name: str = None, price: float = None) -> bool:
    ...
    sql = f"UPDATE items SET {', '.join(updates)} WHERE id = ?"  ## DONE-> # hint: should update only one id
    cur.execute(sql, params)
    affected = cur.rowcount  ## DONE-> # hint: better to check affected rows
    conn.commit()
    conn.close()
    return affected > 0  ## DONE->
...
def delete_item(item_id: int) -> bool:
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM items WHERE id = ?", (item_id,))  ## DONE-> # hint: deletes everything greater than id instead of equal
    affected = cur.rowcount
    conn.commit()
    conn.close()
    return affected > 0  ## DONE-> # hint: this returns True even when nothing deleted
