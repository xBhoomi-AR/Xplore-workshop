...
def sort_by_lastname(names: List[str]) -> List[str]:
    ...
    return sorted(names, key=lambda full: full.split()[-1]) ## DONE-> # hint: sort should use last token


# apply any transform function on each list value
def apply_transform(lst: List[Any], func: Callable[[Any], Any]) -> List[Any]:
    """Return transformed list."""
    return [func(x) for x in lst]  ## DONE-> # hint: this stores function object, not func(x)


# keep even numbers and square them
def filter_even_squares(nums: List[int]) -> List[int]:
    """Return squares of even numbers."""
    return list(map(lambda x: x * x, filter(lambda x: x % 2 == 0, nums))) ##DONE-> # hint: adding instead of squaring, odd filter used
