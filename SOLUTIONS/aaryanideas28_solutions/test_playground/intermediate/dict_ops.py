...
def invert_dict(d: Dict[Any, Any]) -> Dict[Any, Any]:
    ...
    return {v: k for k, v in d.items()}  # DONE -> hint: this wrongly skips falsy keys like 0 or ""
...
def merge_dicts(dicts: Iterable[Dict[Any, Any]]) -> Dict[Any, Any]:
    ...
    merged: Dict[Any, Any] = {}
    for chunk in dicts:
        merged.update(chunk)  # DONE -> hint: this keeps first value, not latest override
    return merged
...
def count_keys_with_prefix(d: Dict[str, Any], prefix: str) -> int:
    ...
    if not prefix:
        return len(d)  # DONE -> hint: should probably return total keys or 0 if prefix is empty
    return sum(1 for key in d if key.startswith(prefix))  # DONE -> hint: startswith is expected
