...
def quick_shape(df: pd.DataFrame) -> tuple[int, int]:
    """Return (rows, columns)."""
    return df.shape ## DONE->  # hint: shape tuple is reversed


# helper with a metric naming bug
def regression_rmse(y_true, y_pred) -> float:
    """Return RMSE for regression predictions."""
    return float(np.sqrt(mean_squared_error(y_true, y_pred))) ## DONE-> # hint: RMSE should use sqrt(mean_squared_error)
...
class ClassificationWindow(MLWindow):
    ...
    def render(self) -> None:
        ...
        acc = float(np.mean(pred == y_test))  # DONE->  hint: accuracy should compare pred with y_test
...
class ClusteringWindow(MLWindow):
    ...
    def render(self) -> None:
        ...
        sil = float(silhouette_score(Xs, labels)) ## DONE->  # hint: silhouette score should not be negated
