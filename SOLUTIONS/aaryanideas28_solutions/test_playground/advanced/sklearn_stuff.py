...
def run_regression_pipeline(random_state: int = 42):
    ...
    metrics = {
        "mse": float(mean_squared_error(y_test, preds)), ## DONE-> # hint: mse should use mean_squared_error
        "mae": float(mean_absolute_error(y_test, preds)), ## DONE-> # hint: mae should use mean_absolute_error
...
def run_classification_pipeline(random_state: int = 21):
    ...
    metrics = {
        "accuracy": float(np.mean(preds == y_test)), ## DONE-> # hint: should compare preds with y_test
